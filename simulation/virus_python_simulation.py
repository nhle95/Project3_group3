...
covid19_params = {
    "r0":2.28, # measure of contagiousness of disease (expected number to spread per person)
    "incubation":5, # average time of incubation
    "percent_mild":0.8, # what percentage of symptoms are mild
    "mild_recovery":(7,14), # how quickly people will recover from mild symptoms (two cases: fast and slow)
    "percent_severe": 0.2, # what percentage of symptoms are severe
    "severe_recovery":(21,42), # how quickly people will recover from severe symptoms (two cases: fast and slow)
    "severe_death":(14,56), # if people die, how quickly will they die (two cases: fast and slow)
    "fatality_rate": 0.034, # what percentage of severe symptoms die
    "serial_interval":7 #average time between successive cases
}
...
def main():
    coronavirus = Virus(covid19_params) # create the Virus object
    coronavirus.animate() # create the animation

    # writervideo = my_anim.FFMpegWriter(fps=60)
    # my_anim.save("/Users/xenialiu/Desktop")
    # f = r"/Users/xenialiu/Desktop/animation.mp4" 
    # writervideo = ani.FFMpegWriter(fps=60) 
    # myanim.save(f, writer=writervideo)
    
    # my_animation.save('animation.gif', writer=writervideo)
    plt.show() # show the animation
    # ani.save("/Users/xenialiu/Desktop/animation.mp4")
